...
#### `Color.createGradientThrough(colors: Array<Color | ColorFormat | string>, options?: ColorGradientOptions): Color[]`
...
#### `Color.createGradientTo(color: Color | ColorFormat | string, options?: Omit<ColorGradientOptions, 'interpolation'>): Color[]`
...
#### `getColorSwatch(options?: ColorSwatchOptions): ColorSwatch | ExtendedColorSwatch`

- <ins>Returns</ins> a [`ColorSwatch`](#types-color) of lighter and darker variants keyed `100`–`900`, reporting the anchored stop via `mainStop`. When `extended` is `true`, half-steps `50`–`950` are added without moving the base color to a half-stop.
- <ins>Inputs</ins>:
  - `options` (optional) - `ColorSwatchOptions`:
    - `centerOn500` - force the original color onto the `500` stop instead of dynamically anchoring by lightness.
    - `extended` - set to `true` to include midpoint stops `50`–`950`.
...
- <ins>Returns</ins> a [`ColorPalette`](#types-color) containing the primary swatch, secondary swatches from the chosen harmony, neutral/tinted neutrals, and semantic swatches (`info`, `positive`, `negative`, `warning`, `special`).
- <ins>Inputs</ins>:
  - `harmony` (optional) - [`ColorHarmony`](#types) used to generate secondary colors (defaults to `'COMPLEMENTARY'`).
