...
const LAB_DELTA_CUBED = LAB_DELTA ** 3; // = 0.00885645
const LAB_F_LINEAR_COEFFICIENT = 1 / (3 * LAB_DELTA ** 2); // = 7.78703704
...
export function toRGB(color: ColorFormat): ColorRGB {
  ...
  switch (formatType) {
    ...
    case 'HWB':
      return hwbToRGB(value);
    case 'HWBA':
      return rgbaToRGB(hwbToRGBA(value));
    case 'CMYK':
