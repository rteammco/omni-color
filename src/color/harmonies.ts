...
import type { ColorFormat, ColorHSLA, ColorRGBA } from './formats.types';
...
function spinLightness(hsla: ColorHSLA, degrees: number): ColorHSLA {
...
function spinColorOnHueOrLightness(
  ...
  if (hsla.s === 0) {
    ...
    return hsla; // No change for grayscale in IGNORE mode (treating the operation as undefined)
  }
...
export function getAnalogousHarmonyColors(
  rgba: Readonly<ColorRGBA>,
  options: ColorHarmonyOptions = {},
): [ColorFormat, ColorFormat, ColorFormat, ColorFormat, ColorFormat] {
  const grayscaleHandlingMode = resolveGrayscaleHandlingMode(options);
...
export function getMonochromaticHarmonyColors(
  ...
  const hsla = toHSLA(rgba);
  const lighter = { ...hsla, l: clampValue(hsla.l + 20, 0, 100) };
  const darker = { ...hsla, l: clampValue(hsla.l - 20, 0, 100) };
  const saturated = { ...hsla, s: clampValue(hsla.s + 20, 0, 100) };
  const desaturated = { ...hsla, s: clampValue(hsla.s - 20, 0, 100) };
