...
import {
  getColorFromTemperature as getColorRGBAFromTemperature,
  getColorFromTemperatureLabel as getColorRGBAFromTemperatureLabel,
  getColorTemperature as getColorTemperatureFromRGBA,
  getColorTemperatureString as getColorTemperatureStringFromRGBA,
  matchPartialColorTemperatureLabel,
} from '../temperature';

function getColorTemperature(color: Color | Readonly<ColorRGBA>) {
  return getColorTemperatureFromRGBA(color instanceof Color ? color.toRGBA() : color);
}

function getColorTemperatureString(
  color: Color | Readonly<ColorRGBA>,
  options: Parameters<typeof getColorTemperatureStringFromRGBA>[1] = {},
) {
  const rgba = color instanceof Color ? color.toRGBA() : color;
  return getColorTemperatureStringFromRGBA(rgba, options);
}

function getColorFromTemperature(temperature: number): Color {
  return new Color(getColorRGBAFromTemperature(temperature));
}

function getColorFromTemperatureLabel(
  label: Parameters<typeof getColorRGBAFromTemperatureLabel>[0],
): Color {
  return new Color(getColorRGBAFromTemperatureLabel(label));
}
...
describe('getColorFromTemperature', () => {
  it('returns a raw RGBA color format', () => {
    expect(getColorRGBAFromTemperature(1500)).toEqual({ r: 255, g: 108, b: 0, a: 1 });
  });
